/*
	...
	Program Features:
	   ...
	   3, Calculating tf*idf with math engine.
*/
...
void Welcome_LSI()
{
    ...
    printf("Please put the 七律wp-id.xlsx into your Matlab catalog.\n");
...
int Connect_Engine_LSI() 
{
	...
	engEvalString(ep, "X = xlsread('七律wp-id.xlsx');"); 
	engEvalString(ep, "w = zeros(7474,1);");
	engEvalString(ep, "k = 1");
	for (int i = 1; i <= RELATIONNUM ; ++i)
	{
		engEvalString(ep, "w(X(k,2),1) = w(X(k,2),1) + 1;");
		engEvalString(ep, "k = k + 1");
	}
	engEvalString(ep, "k = 1");
	for (int i = 1; i <= RELATIONNUM ; ++i)
	{
		engEvalString(ep, "X(k,3) = X(k,3)*log( 472 / w(X(k,2),1));");
		engEvalString(ep, "k = k + 1");
	}
...
void Work_LSI()
{
	...
	else 
	{
		...
		//	for (int i=0 ; i <= POEMSNUM-1 ; ++i) printf("%d\n",vresult[i]);
	}
}
...
int Feedback_LSI(int x)
{
	...
	printf("Press 2 to see the synonyms.\n");
